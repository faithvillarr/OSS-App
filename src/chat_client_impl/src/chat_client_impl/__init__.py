...
def get_client_impl(user_id: str | None = None) -> chat_api.ChatInterface:
    ...
    if bot_token:
        # "Bot" is a Discord API token type identifier, not a password
        token_type_value = "Bot"  # noqa: S105
        return ChatClient(access_token=bot_token, token_type=token_type_value)
    return ChatClient(user_id=user_id)
