                    ...
                    "properties": {
                        ...
                        "params": {
                            ...
                            "properties": {
                                ...
                                "title": {
                                    "type": ["string", "null"],
                                    "description": "The title of the ticket (required for create_ticket, optional for update_ticket)"
                                },
                                "description": {
                                    "type": ["string", "null"],
                                ...
                                "query": {
                                    "type": ["string", "null"],
                                    "description": "Search query to filter tickets by title/description (optional for search_tickets)"
                                },
                                "assignee": {
                                    "type": ["string", "null"],
                            ...
                            # of which fields are required for which command types is handled by our _validate_command_response function
                        },
                    },
...
- Ensure all required fields are present and correctly formatted"""
            ...
            logger.exception("Failed to extract commands from user message (attempt %d/%d): %s", 
...
Generate a helpful response to the user."""
        ...
        return "I processed your request, but I'm having trouble generating a response."
    except Exception as e:
...
- Ensure all required fields are present and correctly formatted"""
            ...
            logger.exception("Failed to correct commands (attempt %d/%d): %s", 
                           attempt + 1, max_retries, e)
...
def execute_commands_iteratively(
    user_message: str,
    initial_commands: list[dict[str, Any]],
    ticket_client: Any,  # TicketInterface
    ...
    while pending_commands and iteration < max_iterations:
        ...
        if iteration < max_iterations:
            ...
            context_message = ""
            if pending_commands:
                context_message = f"There are {len(pending_commands)} more command(s) from the initial batch waiting to be executed. However, you may need to generate follow-up commands that use data from the results above. If a pending command needs data from previous results, you can generate a new command with that data instead."
            else:
                context_message = "All initial commands have been executed. Determine if the user's request is complete or if follow-up commands are needed based on the results above."
...
- Search results show tickets: [{{"id": "abc123", "title": "Cloud Components", ...}}, {{"id": "def456", "title": "Other Task", ...}}]
...
If the request is fully satisfied, return an empty actions array []"""
            ...
            try:
                ...
                logger.exception("Error generating follow-up commands (iteration %d): %s", iteration, e)
                # Continue with any remaining pending commands
                if not pending_commands:
                    break
    ...
    for i, result in enumerate(results, 1):
        ...
        if result.get("success"):
            ...
            if ticket_data:
                ...
                lines.append(f"  Success: True (no data returned)")
        else:
...
def _validate_command_response(response: Any) -> tuple[bool, str | None]:
