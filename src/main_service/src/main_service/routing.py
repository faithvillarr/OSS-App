...
def _build_user_input_with_errors(
    base_message: str, validation_errors: list[str]
) -> str:
    """Build user input with validation error feedback.

    Args:
        base_message: The base user message.
        validation_errors: List of validation error messages.

    Returns:
        User input string with error feedback if there are errors.
...
def _process_valid_response(
    ...
    """Process a valid AI response and extract actions.
    ...
    Returns:
        List of actions if successful, None if validation fails.

    """
    if not isinstance(response, dict):
        return None

    actions = cast("list[dict[str, Any]]", response.get("actions", []))
    if attempt > 0:
        logger.info("%s: Successfully extracted %d command(s) after %d retry attempt(s)",
                  log_prefix, len(actions), attempt)
    else:
        logger.info("%s: Extracted %d command(s)", log_prefix, len(actions))

    # Log each extracted command with details
    for i, action in enumerate(actions, 1):
        if isinstance(action, dict):
            logger.info("%s: Extracted command %d: type=%s, params=%s",
                      log_prefix, i, action.get("type"), action.get("params", {}))
...
def extract_commands(user_message: str) -> list[dict[str, Any]]:
    ...
    for attempt in range(max_retries):
        try:
            user_input = _build_user_input_with_errors(user_message, validation_errors)
            ...
            if is_valid and isinstance(response, dict):
                actions = _process_valid_response(
                    response, attempt, "Command extraction"
                )
...
Please provide corrected commands."""
    ...
    for attempt in range(max_retries):
        try:
            user_input = _build_user_input_with_errors(base_user_input, validation_errors)
            ...
            if is_valid and isinstance(response, dict):
                actions = _process_valid_response(
                    response, attempt, "Command correction"
                )
                if actions is not None:
                    return actions
                validation_errors.append("Response is not a dictionary")
...
If the request is fully satisfied, return an empty actions array []"""
            ...
            try:
                ...
                if is_valid:
                    # Type guard: response is valid dict[str, Any] at this point
                    if not isinstance(response, dict):
                        logger.warning("Response is not a dictionary, skipping follow-up")
                        break
...
def _validate_command_response(  # noqa: C901, PLR0911
    response: dict[str, Any] | list[Any] | str | float | bool | None,  # noqa: FBT001
) -> tuple[bool, str | None]:
