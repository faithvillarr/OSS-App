                        ...
                        "params": {
                            ...
                            "properties": {
                                ...
                                "title": {
                                    "type": ["string", "null"],
                                    "description": (
                                        "The title of the ticket (required for create_ticket, optional for update_ticket)"
                                    ),
                                },
                                "description": {
                                    "type": ["string", "null"],
                                ...
                                "query": {
                                    "type": ["string", "null"],
                                    "description": (
                                        "Search query to filter tickets by title/description (optional for search_tickets)"
                                    ),
                                },
                                "assignee": {
                                    "type": ["string", "null"],
                            ...
                            # validation of which fields are required for which command types
                            # is handled by our _validate_command_response function
                        },
                    },
    ...
    for attempt in range(max_retries):
        try:
            ...
            if validation_errors:
                error_feedback = "\n\n".join(
                    [f"Previous attempt {i + 1} failed: {error}" for i, error in enumerate(validation_errors)]
                )
...
def generate_response(user_message: str, ticket_results: list[dict[str, Any]]) -> str:
...
def correct_commands(original_commands: list[dict[str, Any]], error_message: str) -> list[dict[str, Any]]:
...
- Ensure all required fields are present and correctly formatted"""
            ...
            if attempt == max_retries - 1:
                logger.error(
                    "Failed to correct commands after %d attempts. Errors: %s. Returning original commands.",
                    max_retries,
                    validation_errors,
                )
            ...
            logger.exception("Failed to correct commands (attempt %d/%d)", attempt + 1, max_retries)
...
def execute_commands_iteratively(  # noqa: C901, PLR0912, PLR0915
    ...
    ticket_client: Any,  # TicketInterface  # noqa: ANN401
    ...
    logger.info(
        "Starting iterative command execution with %d initial command(s), max %d iterations",
    ...
    while pending_commands and iteration < max_iterations:
        ...
        if iteration < max_iterations:
            ...
            if pending_commands:
                ...
                context_message = (
                    "All initial commands have been executed. Determine if the user's request is "
                    "complete or if follow-up commands are needed based on the results above."
                )
