...
def _test_update_ticket_title(client: TicketsClient, ticket_id: str) -> str | None:
    ...
    try:
        updated_ticket = client.update_ticket(ticket_id=ticket_id, title="Test Ticket - Updated Title")
...
def _test_list_all_tickets(client: TicketsClient) -> None:
    ...
    try:
        ...
        for i, ticket in enumerate(all_tickets[:5], 1):  # Show first 5
            status_emoji = "✓" if ticket.status == TicketStatus.CLOSED else "○" if ticket.status == TicketStatus.OPEN else "→"
            logger.info(
                "  %d. %s %s (ID: %s, Status: %s)",
                i,
                status_emoji,
