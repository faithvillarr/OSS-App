...
class TestRoutingTicketingIntegration:
    ...
    def test_generate_response_from_ticket_results(
        ...
        mock_ai_client.generate_response.return_value = "I've successfully created ticket ticket_123: Test Ticket"
...
class TestTicketsClientMethodsIntegration:
    ...
    def test_tickets_client_search_with_status_filter(
        ...
        mock_tickets_client.search_tickets.assert_called_once_with(query=None, status=TicketStatus.IN_PROGRESS)
        assert results[0]["success"] is True
    ...
    def test_tickets_client_update_ticket_title_only(
        ...
        mock_tickets_client.update_ticket.assert_called_once_with(ticket_id="ticket_123", status=None, title="New Title")
        assert results[0]["success"] is True
    ...
    def test_tickets_client_update_ticket_status_only(
        ...
        mock_tickets_client.update_ticket.assert_called_once_with(ticket_id="ticket_123", status=TicketStatus.CLOSED, title=None)
        assert results[0]["success"] is True
...
class TestGTaskClientThroughTicketsClient:
    ...
    def test_insert_task_error_path(
        ...
        mock_tickets_client._gtask_client = MagicMock()
        mock_tickets_client._gtask_client.insert_task.side_effect = HttpError(resp=MagicMock(status=400), content=b"Bad Request")
